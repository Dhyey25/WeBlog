...
const SearchButton = () => {
  ...
  return (
    <div className="">
      <button
        className="outline-2 outline outline-dark hover:outline-offset-2 p-2 rounded-full"
      ...
      <Modal
        ...
        className="flex justify-center items-start w-full h-5/6 mt-24"
      >
...
const SearchBar: React.FC<SearchBarProps> = ({ handleModalClose }) => {
  ...
  return (
    <div className="bg-white w-11/12 md:w-[55%] rounded-2xl p-5 md:p-10 outline-none relative">
      <form
        onSubmit={(e) => {
          e.preventDefault()
      ...
      >
        <input
          ...
          className="w-full p-3 px-4 md:px-6 md:text-lg outline-none bg-secondary/30 rounded-full text-dark font-medium placeholder-gray-500"
      ...
      <div className="md:px-5 overflow-y-scroll h-[90%]">
        {inputValue.length >= 1 && (
          <p className="absolute top-12 right-12 text-gray-600 text-xs hidden md:block">
            <span>Press</span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            ...
            >
              <path d="M19,6a1,1,0,0,0-1,1v4a1,1,0,0,1-1,1H7.41l5.3-5.29a1,1,0,0,0-1.42-1.42l-7,7a1,1,0,0,0,0,1.42l7,7a1,1,0,0,0,1.42,0,1,1,0,0,0,0-1.42L7.41,13H18a3,3,0,0,0,3-3V7A1,1,0,0,0,19,6Z"></path>
            </svg>
        ...
        {inputValue.length === 0 && (
          <p className="flex items-center gap-1 justify-center pt-20 text-gray-500 font-medium">
            <SearchSvg />
              ...
              <div>
                {data?.map((item: any, index: number) => (
                  ...
                  >
                    <img
                      src={item?.img}
                      className="aspect-video object-cover h-20 w-36 rounded-md bg-gray-300"
                    />
                    <div className="flex flex-col gap-1">
                      <h3 className="md:text-lg font-semibold text-gray-800 line-clamp-1">
                        {item?.title}
                      </h3>
                      <p className="hidden md:block text-gray-600 text-sm line-clamp-1">
                        {item.description}
                      </p>
          ...
          <div className="flex flex-col gap-7 ">
            ...
            {[1, 2].map((i) => (
              <div
                key={i}
                role="status"
                className="max-w-sm animate-pulse flex gap-4"
              >
                <div className="h-20 bg-gray-300 rounded w-36"></div>
        ...
        {inputValue.length >= 3 && !isLoading && data.length === 0 && (
          <p className="text-sm text-gray-500 md:text-base mt-4 px-5 italic">
