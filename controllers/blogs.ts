...
const getBlogByCategory = async (req: Request, res: Response) => {
    ...
    const blogs = await Blog.find(query)
        ...
        .sort({ createdAt: -1 })
        .select("title description img author tags views likesCount commentsCount createdAt")
        .populate({
            path: "author",
...
const getUserBlogs = async (req: Request, res: Response) => {
    const userId = getUserId(req)
    const userBlogs = await Blog.find({ author: userId as any })
        .skip(req.pagination.skip)
        .limit(req.pagination.limit)
        .sort({ createdAt: -1 })
    ...
    const totalCount = await Blog.countDocuments({ author: userId as any })
...
const getUserBlogById = async (req: Request, res: Response) => {
    ...
    const blog = await Blog.findOne({ _id: blogId, author: userId as any})
    if (!blog)
...
const createBlog = async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(tags))
        throw new BadRequestError("Tags should be an array of strings")
    ...
    await User.findByIdAndUpdate(userId, {
        $push: { blogs: (blog as any)._id  },
    })

    res.status(StatusCodes.CREATED).json({
        data: { id: (blog as any)._id },
        success: true,
...
const deleteBlog = async (req: Request, res: Response) => {
    ...
    if (blogIndex === -1)
        return res.status(404).json({ error: "Blog not found in user logs" })
